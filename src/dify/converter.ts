...
export function convertDifyWorkflowToMCPTools(infoData: DifyInfoResponse, paramsData: DifyParametersResponse): Tool[] {
  const tools: Tool[] = [];
  
  // Extract necessary information from infoData and paramsData to create tool definitions
  if (infoData && paramsData) {
    // Example: Use workflow name as tool name
    const toolName = infoData.name || "dify-workflow";
    ...
    console.error("Error: infoData or paramsData is missing");
  }
